...
serve(async (req) => {
  ...
  try {
    ...
    if (payload.event_type === "payment.succeeded") {
      ...
      if ((status === "completed" || status === "success" || status === "succeeded") && metadata?.user_id) {
        ...
        const { data: payment, error: paymentError } = await supabaseAdmin
          ...
          .in("status", ["pending", "processing"]) // be tolerant to intermediate states
          .single();
        ...
        // Found payment record (sanitized)
